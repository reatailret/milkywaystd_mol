namespace $.$$
{
	...
	export class $mws_crud_demo_products_service extends $mws_crud<Product> {
		private static _intances = new Map()
		@$mol_mem_key
		static getInstance<T1 extends typeof $mws_crud<Product>>( this: T1, id: string = 'default' ): InstanceType<T1>
		{
			if( !$mws_crud_demo_products_service._intances.get( id ) )
			{
				...
				$mws_crud_demo_products_service._intances.set( id, srv )
			}

			return $mws_crud_demo_products_service._intances.get( id )
		}
	}
}
